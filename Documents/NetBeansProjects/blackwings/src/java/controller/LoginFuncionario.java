        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try {
                ...
                if (f.getId() > 0) {
                    ...
                    if(f.getPerfil().getTitulo().equals("Atendente")){
                        response.sendRedirect("agendamento.jsp");
                    }else if(f.getPerfil().getTitulo().equals("Barbeiro")){
                        response.sendRedirect("barbeiro.jsp");
                    }else if(f.getPerfil().getTitulo().equals("Gerente") || f.getPerfil().getTitulo().equals("Administrador")){
                        response.sendRedirect("administracao.jsp");
                    }
                    
                } else {
                    response.sendRedirect("login_funcionario.jsp?msg=1");
//                       
                }
